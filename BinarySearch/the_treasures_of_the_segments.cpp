...
void solve()
{
    ...
    {
        cin >> left[i] >> right[i];
        pairs[i].first = left[i];
        pairs[i].second = right[i];
    }
    ...
    for(int i = 0;i<n;i++)
    {
        ll lsats = lower_bound(right.begin(),right.end(),pairs[i].first) - right.begin();
        ll rsats = n - (upper_bound(left.begin(),left.end(),pairs[i].second) - left.begin());
        rsats = max(rsats,(ll)0);
        ans = min(ans,lsats+rsats);
    }
    cout << min((ll)ans,n-1) << endl;
}
