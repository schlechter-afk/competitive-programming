...
node query(int l, int r, int minrange, int maxrange, int idx)
{
    ...
    int mid = (l + r)/2;
    node lquery = query(l,mid,minrange,min(maxrange,mid),2*idx+1);
    node rquery = query(mid+1,r,max(minrange,mid+1),maxrange,2*idx+2);
