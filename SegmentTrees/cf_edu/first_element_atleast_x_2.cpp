...
node first_above(int k,int l, int r, int idx,int minli, int maxli)
{
    if(l > maxli || r < minli) return -1;
    if(l >= minli && r <= maxli)
    {
    ...
    if(l==r) return l;
    int mid = (l+r)/2;
    int res = first_above(k,l,mid,2*idx+1,minli,maxli);
    if(res == -1)
    {
        res = first_above(k,mid+1,r,2*idx+2,minli,maxli);
    }
    return res;
}
...
void solve()
{
    ...
    while(m--)
    {
        ...
        else
        {
            ...
            cin >> val >> idx;
            // cout << store[idx] << "**\n";
            cout << first_above(val,0,n-1,0,idx,n-1) << endl;
        }
    }
}
