...
{
    if(r < minrange || l > maxrange) return 0;
...
node findind(int l, int r, int idx, int k)
{
    if(l == r)
    {
        if(segarr[idx]==k) return l;
        else return N;
    }
    int mid = (l+r)/2;
    if(segarr[2*idx+1]>=k)
    {
        return findind(l,mid,2*idx+1,k);
    }
    else{
        return findind(mid+1,r,2*idx+2,k-segarr[2*idx+1]);
    }
} 
...
void solve()
{
    ...
    for(int i=n-1;i>=0;i--)
    {
        int idx = findind(0,n-1,0,segarr[0]-arr[i]);
        ans[i] = idx+1;
        setval(max(idx,(ll)0),0,0,0,n-1);
    }
