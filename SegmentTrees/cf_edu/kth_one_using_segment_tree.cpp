...
node find(int idx, int l, int r, int k)
{
    if(l == r) return l;
    int mid = (l + r)/2;
    int val = segarr[2*idx+1];
    if(k<val) return find(2*idx+1,l,mid,k);
    else return find(2*idx+2,mid+1,r,k-val);
}
...
void solve()
{
    ...
    while(m--)
    {
        int ty,a;
        cin >> ty >> a;
        if(ty==1) setval(a,1-arr[a],0,0,n-1);
        else cout << find(0,0,n-1,a) << endl;
    }
}
