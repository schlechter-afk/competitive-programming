...
pa query(int l, int r, int minrange, int maxrange, int idx)
{
    if(r < minrange || l > maxrange) return {INT_MAX,0};
    if(l >= minrange && r <= maxrange)
    {
        return segarr[idx];
    }
    int mid = (l + r)/2;
    pa lquery = query(l,mid,minrange,maxrange,2*idx+1);
    pa rquery = query(mid+1,r,minrange,maxrange,2*idx+2);
