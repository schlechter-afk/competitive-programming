...
{
    node ans;
    ans.seg = max(a.seg,max(b.seg,a.suff+b.pref));
    ans.pref = max(a.pref,a.sum_all+b.pref);
    ans.suff = max(b.suff,b.sum_all+a.suff);
    ans.sum_all = a.sum_all + b.sum_all;
    return ans;
}
...
{
    if(r < minrange || l > maxrange) return {0,0,0,0};
...
void solve()
{
    ...
    init(n+1);
    build(0,n-1,0);
    cout << query(0,n-1,0,n-1,0).seg << endl;
    while(m--)
    {
        int a,b;
        cin >> a >> b;
