...
void solve()
{
    ll n;
    cin >> n;
    vector<pair<pa, int>> vec(n);
    for (int i = 0; i < n; i++)
    {
        cin >> vec[i].first.first >> vec[i].first.second;
        vec[i].second = i;
        // cout << i << " " << vec[i].first.first << " " << vec[i].first.second << " ";
    }
    sort(vec.begin(), vec.end());
    priority_queue<pa, vp, greater<pa>> pq;
    ll rooms = 0;
    ll allot_room = 0;
    vi ans(n);
    for (int i = 0; i < n; i++)
    {
        ...
        {
            allot_room++;
            // cout << vec[i].first.second << " " << i << " " << "****\n";
            pq.push({vec[i].first.second, allot_room});
            ans[vec[i].second] = allot_room;
        }
        else
        {
            pa top = pq.top();
            if (top.first < vec[i].first.first)
            {
                pq.pop();
                // cout << i << " " << top.second << "*\n";
                pq.push({vec[i].first.second, top.second});
                ans[vec[i].second] = top.second;
            }
            else
            {
                allot_room++;
