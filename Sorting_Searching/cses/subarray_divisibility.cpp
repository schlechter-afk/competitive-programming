...
#define read(vec) for(int i = 0;i<vec.size();i++) {cin >> vec[i];}
#define output(vec) for(int i = 0;i<vec.size();i++) {cout << vec[i];}
template <typename T1, typename T2> pair<T1, T2> operator+(pair<T1, T2> &l, pair<T1, T2> &r) { return {l.first + r.first, l.second + r.second}; }
