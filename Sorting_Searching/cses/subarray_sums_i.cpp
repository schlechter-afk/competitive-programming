...
struct chash
{
    ...
    int operator()(ll x) const { return hash_f(x) ^ RANDOM; }
};

typedef cc_hash_table<ll, ll, hash<ll>> ht;
typedef gp_hash_table<ll, ll, chash> gptable;
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        curr_sum += vec[i];
        if(curr_sum == x) ans += 1;
        if(container.find(curr_sum - x) != container.end()) ans += container[curr_sum - x];
        container[curr_sum]++;
    }
    cout << ans;
}
