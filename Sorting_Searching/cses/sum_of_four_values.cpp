...
void solve()
{
    ...
    vi vec(n);
    read(vec);
    gp_hash_table<int, pa, chash> gptable;

    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++)
        {
            int val = x - vec[i] - vec[j];
            if (gptable.find(val) != gptable.end())
            {
                cout << i + 1 << " " << j + 1 << " " << gptable[val].first + 1 << " " << gptable[val].second + 1;
                return;
            }
        }
        for (int j = 0; j < i; j++)
        {
            gptable[vec[i] + vec[j]] = {i, j};
        }
    }
    cout << "IMPOSSIBLE";
}
