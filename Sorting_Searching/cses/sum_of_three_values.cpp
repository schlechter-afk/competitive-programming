...
// use chash : tl 0.01s(with) vs 0.72s (without) when order of 1e9.
typedef cc_hash_table<ll, ll, hash<ll>> ht;
typedef gp_hash_table<int, int, chash> gpint;
typedef gp_hash_table<int, pa, chash> gppair;
...
void solve()
{
    ...
    // gptable blows up, problem creators have done a good job setting tight tl cases : anti hash tests ;((
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        while (l < r)
        {
            int targ = x - vec[i].first;
            if (l != i && r != i && targ == vec[l].first + vec[r].first)
            {
                cout << vec[l].second << " " << vec[r].second << " " << vec[i].second;
                return;
            }
            if (targ > vec[l].first + vec[r].first) l++;
            else r--;
        }
    }
    cout << "IMPOSSIBLE";
