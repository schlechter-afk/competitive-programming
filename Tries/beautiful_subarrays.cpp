...
void insert(Node *node, int val)
{
    ...
    for(int i = 16; i >= 0; i--)
    {
        ...
        root->links[bit]->cnt += 1;
        root = root->links[bit];
    }
}

int xorless(Node* node,int value,int lim)
{
    Node* root = node;
    int ans = 0;
    ...
    {
        bool limbit = (1 << i) & lim;
        bool valbit = (1 << i) & value;
        if(limbit){
            root = root->links[1 - valbit];
        }
        else{
            if(root->links[1 - valbit]){
                ans += root->links[1 - valbit]->cnt;
            }
            root = root->links[valbit];
        }
    }
    return ans;
}
