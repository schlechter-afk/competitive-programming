...
void solve()
{
    ...
    // dp[i] denotes minimum weight with which we can make total value i.

    vector<ll> dp(tot_val + 1, 1e12);
    dp[0] = 0;
    for (int i = 1; i <= N; i++)
    {
        for (int j = tot_val; j >= 0; j--)
        {
            if(j >= values[i]) dp[j] = min(dp[j], dp[j - values[i]] + weights[i]);
            else break;
        }
    }
    ll ans = 0;
