...
void solve()
{
    ...
    cin >> n;
    vi vec(n);
    int cntones = 0,cnttwos=0,cntthrees = 0;
    for (int i = 0; i < n; i++)
    {
        cin >> vec[i];
        if(vec[i]==1) cntones++;
        if(vec[i]==2) cnttwos++;
        if(vec[i]==3) cntthrees++;
    }
    ...
    for (int i = 0; i <= n; i++)
    {
        for (int j = 0; j <= n; j++)
        {
            for (int k = 0; k <= n; k++)
            {
                ...
                double ijkval = 1;
                if(i) ijkval += (double)(dp[i-1][j+1][k]*(i/(double)n));
                if(j) ijkval += (double)(dp[i][j-1][k+1]*(j/(double)n));
                if(k) ijkval += (double)(dp[i][j][k-1]*(k/(double)n));
                ijkval /= (double)(1-((double)1.0*(n-i-j-k)/(double)n));
                dp[i][j][k] = (double)ijkval;
            }
        }
    }
    cout << setprecision(16) << fixed << (double)dp[cntthrees][cnttwos][cntones];
}
