...
void solve()
{
    ll n;
    cin >> n;
    vector<array<int, 3>> vacations(n);
    for (int i = 0; i < n; i++)
    {
        array<int, 3> arr;
    ...
    for (int i = 1; i < n; i++)
    {
        dp[i][0] = max(dp[i - 1][1], dp[i - 1][2]) + vacations[i][0];
        dp[i][1] = max(dp[i - 1][0], dp[i - 1][2]) + vacations[i][1];
        dp[i][2] = max(dp[i - 1][0], dp[i - 1][1]) + vacations[i][2];
    }
