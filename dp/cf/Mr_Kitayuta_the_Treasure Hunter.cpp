...
int recurse(register int idx,register int jump)
{
    if(idx > 30000) return 0;
    register int offset = jump - (d-247);
    if(dp[idx][offset] != -1) return dp[idx][offset];
    register int val;
    if(jump == 1) val = mp[idx] + max(recurse(idx+jump,jump),recurse(idx+jump+1,jump+1));
    else val = mp[idx] + max(recurse(idx+jump-1,jump-1),max(recurse(idx+jump,jump),recurse(idx+jump+1,jump+1)));
    return dp[idx][offset] = val;
}
