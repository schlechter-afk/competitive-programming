...
ll solve(ll l, ll r, ll offset, vi &vec)
{
    if (l == r)
        return 1;
    ll minel = INT_MAX;
    for (int i = l; i <= r; i++)
    {
        minel = min(minel, vec[i] - offset);
    }
    ll ans = minel;
    ll breakidx = l;
    for (int i = l; i <= r; i++)
    {
        if (vec[i] - offset == minel)
        {
            if (i != breakidx)
            {
                ans += solve(breakidx, i - 1, offset + minel, vec);
            }
            breakidx = i + 1;
        }
    }
    if (vec[r] - offset != minel)
    {
        ans += solve(breakidx, r, offset + minel, vec);
    }
