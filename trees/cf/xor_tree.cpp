...
ll recurse(vi &vec, int bit)
{
    if(!vec.size()) return 0;
    if(bit < 0) return vec.size();
    ll lim = 1LL << bit;
    vi low,high;
    for(auto ele:vec) if(ele & lim) low.pb(ele);else high.pb(ele);
    if(sz(high) > 1 && sz(low) > 1) return 1+max(recurse(low,bit-1),recurse(high,bit-1));
    else if(sz(low) == 1) return 1+recurse(high,bit-1);
    else if(sz(high) == 1) return 1+recurse(low,bit-1);
    else if(!sz(low)) return recurse(high,bit-1);
    else if(!sz(high)) return recurse(low,bit-1);
    else return recurse(high,bit-1);
    return 0;
}
...
void solve()
{
    ...
    {
        cin >> vec[i];
        maxel = max(maxel,vec[i]);
    }
    int bit = 30;
    while(((1LL << bit) & maxel) == 0) bit--;
    cout << n - recurse(vec,bit);
}
